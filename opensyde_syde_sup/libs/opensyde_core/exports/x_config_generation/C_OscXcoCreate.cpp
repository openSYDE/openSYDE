                                        ...
                                        const stw::scl::C_SclString & orc_TemporaryDirectory)
{
   ...
   if (s32_Return == C_NO_ERR)
   {
      C_OscSpaServicePackageCreateUtil::h_GetTempFolderName(orc_PackagePath, orc_TemporaryDirectory,
                                                            "Creating Update Package", hc_PACKAGE_EXT,
                                                            hc_PACKAGE_EXT_TMP, c_PackagePathTmp);

      //erase target path if it exists:
      if (TglDirectoryExists(c_PackagePathTmp) == true)
      {
         s32_Return = TglRemoveDirectory(c_PackagePathTmp, false);
      }

      if (s32_Return == C_NO_ERR)
      {
         //create target folder (from bottom-up if required):
         s32_Return = C_OscUtils::h_CreateFolderRecursively(c_PackagePathTmp);
      }
      if (s32_Return != C_NO_ERR)
      {
   ...
   if (s32_Return == C_NO_ERR)
   {
      ...
      s32_Return = C_OscUtils::h_CreateFolderRecursively(c_SysDefPath);
      if (s32_Return != C_NO_ERR)
      {
         // very strange! normally the precondition check should
         // guarantee a correct behavior of h_CreateTemporaryFolder
         mhc_ErrorMessage = "Could not create temporary folder \"" +
                            c_SysDefPath + "\" with application files.";
         osc_write_log_error(mhc_USE_CASE, mhc_ErrorMessage);
         s32_Return = C_RD_WR; // redefine because we only have a few error codes
      }
      else
   ...
   if (s32_Return == C_NO_ERR)
   {
      ...
      s32_Return = C_OscUtils::h_CreateFolderRecursively(c_DevDefPath);
      if (s32_Return != C_NO_ERR)
      {
         // very strange! normally the precondition check should
         // guarantee a correct behavior of h_CreateTemporaryFolder
         mhc_ErrorMessage = "Could not create temporary folder \"" +
                            c_DevDefPath + "\" with application files.";
         osc_write_log_error(mhc_USE_CASE, mhc_ErrorMessage);
         s32_Return = C_RD_WR; // redefine because we only have a few error codes
      }
      else
