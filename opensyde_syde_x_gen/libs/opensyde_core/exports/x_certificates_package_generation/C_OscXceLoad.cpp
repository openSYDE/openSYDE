...
/*!
   ...
   openSYDE: X-Certificates Package loading
...
const stw::scl::C_SclString C_OscXceLoad::mhc_USE_CASE = "Unpacking X-Certificates Package";
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief   Unpack an X-Certificates package
   ...
   * content of manifest file as output parameter
   ...
   Assumptions:
   * valid and compatible X-Certificates package created by C_OscXceCreate::h_CreatePackage function
   * write permission of target path for unzipped package

   \param[in]      orc_PackagePath        full path of package
                                          (with constant extension ".syde_xcert", e.g. 012345.syde_xcert)
                                       ...
                                       stw::scl::C_SclString & orc_ErrorMessage)
{
   ...
   {
      const stw::scl::C_SclString c_ManifestPath = c_TargetUnzipPath + C_OscXceManifestFiler::hc_FILE_NAME;
      s32_Return = C_OscXceManifestFiler::h_LoadFile(orc_Manifest, c_ManifestPath);
   }
...
//----------------------------------------------------------------------------------------------------------------------
/*! \brief  This function shall check if certain files are existent in the X-Certificates package. (internal function)

   The update package will be a created directory after unpacking a ".syde_xcert" file,
   In order to successfully import we need to make sure that
   whether the "syde_xcert" got "corrupted". There are some defensive checks for files which are
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscXceLoad::mh_CheckXcertFiles(const stw::scl::C_SclString & orc_PackagePath)
{
   int32_t s32_Return;
