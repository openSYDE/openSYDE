...
{
   bool q_IsRequest;
   uint8 u8_SenderSegment;
   uint8 u8_SenderNode;
   uint8 u8_ReceiverSegment;
   uint8 u8_ReceiverNode;
   uint16 u16_VariableIndex;
   uint16 u16_Data;
   uint16 u16_NumberOfBytes;
   uint32 u32_SysTime;
   C_SCLString c_Text;
   C_SCLString c_Help;
   ...
   case GD_REQUEST_ECRR_WO_HS:
      ...
      if(q_IsRequest == true)
      {
         ...
         {
            c_HelpTime = m_GetValueDecHex(static_cast<uint16>(orc_Msg.au8_Data[4]) * 100) + "ms";
         }
         //evaluate upper hysteresis
         if(orc_Msg.au8_Data[5] <= 99U)
         {
            //0.0% .. 9.9%
            c_HelpHystHigh = m_GetByteAsStringFormat(static_cast<uint8>(orc_Msg.au8_Data[5]/10)) +
                            "." +
                            m_GetByteAsStringFormat(static_cast<uint8>(orc_Msg.au8_Data[5] % 10)) +
                            "%";
         }
         else if((orc_Msg.au8_Data[5] >= 110U) && (orc_Msg.au8_Data[5] <= 200U))
         {
            //10% .. 100%
            c_HelpHystHigh = m_GetByteAsStringFormat(static_cast<uint8>(orc_Msg.au8_Data[5] - 100)) + "%";
         }
         else
         {
         ...
         {
            //0.0% .. 9.9%
            c_HelpHystLow = m_GetByteAsStringFormat(static_cast<uint8>(orc_Msg.au8_Data[6] / 10)) +
                           "." +
                           m_GetByteAsStringFormat(static_cast<uint8>(orc_Msg.au8_Data[6] % 10)) +
                           "%";
         }
         else if((orc_Msg.au8_Data[6] >= 110U) && (orc_Msg.au8_Data[6] <= 200U))
         {
            //10% .. 100%
            c_HelpHystLow = m_GetByteAsStringFormat(static_cast<uint8>(orc_Msg.au8_Data[6] - 100)) + "%";
         }
         else
         {
   ...
   case GD_REQUEST_ECRR_HS:
      ...
      if(q_IsRequest == true)
      {
         ...
         {
            //10% .. 100%
            c_HelpHystHigh = m_GetByteAsStringFormat(static_cast<uint8>(orc_Msg.au8_Data[5] - 100U)) + "%";
         }
         else
         {
            //invalid upper hysteresis
   ...
   case GD_REQUEST_TCRR:
      ...
      if(q_IsRequest == true)
      {
         ...
         {
            c_HelpTime = m_GetValueDecHex(static_cast<uint16>(orc_Msg.au8_Data[4]) * 10U) + "ms";
         }
         else if (orc_Msg.au8_Data[4] >= 101U)
         {
            c_HelpTime = m_GetValueDecHex((static_cast<uint32>(orc_Msg.au8_Data[4]) - 100U) * 1000U) + "sec";
         }
         else
         {
            c_HelpTime = "invalid";
   ...
   case GD_IMMEDITAE_WRITE_HS:
      //immeditate write with handshake
      if (q_IsRequest == true)
      {
         //request
         u16_Data = static_cast<uint16>(orc_Msg.au8_Data[5] + (static_cast<uint16>(orc_Msg.au8_Data[6]) << 8U));
