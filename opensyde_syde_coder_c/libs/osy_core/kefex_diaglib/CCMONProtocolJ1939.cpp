   ...
   if (orc_Msg.u8_XTD == 1)
   {
      ...
      if (u8_PduFormat <= J1939_PDU1_MAX)
      {
         ...
         case J1939_PGN_ADDRESS_CLAIMED:
            ...
            if (static_cast<uint8>(orc_Msg.u32_ID) == J1939_SA_NULL)
            {
               //No address claimed
      ...
      //get source address
      c_SourceAddress += m_GetByteAsStringFormat(static_cast<uint8>(orc_Msg.u32_ID)) + " ";
...
C_SCLString C_CMONProtocolJ1939::m_PgnToString(const uint32 ou32_Pgn) const
{
   ...
   {
      u16_Pos = static_cast<uint16>((u16_First + u16_Last) / 2);
      if (static_cast<uint16>(ou32_Pgn) > mat_Pgn[u16_Pos].u16_Value)
      {
         u16_First = static_cast<uint16>(u16_Pos + 1);
      }
      else if (static_cast<uint16>(ou32_Pgn) < mat_Pgn[u16_Pos].u16_Value)
      {
         u16_Last = static_cast<uint16>(u16_Pos - 1);
      }
      else
      {
         //found PGN
...
C_SCLString C_CMONProtocolJ1939::m_GetName(const T_STWCAN_Msg_RX & orc_Msg) const
{
   ...
   //Identity Number
   c_Data += "IN:";
   c_Data += m_GetValueDecHex(static_cast<uint32>(u64_Name & 0x1FFFFFU));
