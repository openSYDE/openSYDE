                                                            ...
                                                            const std::vector<std::vector<uint8_t> > & orc_NodeSignatureKeys)
{
   ...
   if (s32_Return == C_NO_ERR)
   {
      ...
      c_PackagePathTmp = c_PackagePathTmp.SubString(1, c_PackagePathTmp.Pos(mhc_PACKAGE_EXT) - 1) +
                         mhc_PACKAGE_EXT_TMP;
   ...
   if (s32_Return == C_NO_ERR)
   {
      ...
      const C_SclString c_SysDefPath = c_PackagePathTmp + mhc_SUP_SYSDEF;
                                                                          ...
                                                                          const std::vector<std::vector<uint8_t> > & orc_NodeSignatureKeys)
{
   ...
   if (s32_Return == C_NO_ERR)
   {
      ...
      if ((orc_PackagePath.Pos(mhc_PACKAGE_EXT) == 0) ||
          (orc_PackagePath.Length() != ((orc_PackagePath.Pos(mhc_PACKAGE_EXT) + mhc_PACKAGE_EXT.Length()) - 1)))
      {
         mhc_ErrorMessage = "Package name must have extension \"" + mhc_PACKAGE_EXT + "\".";
         osc_write_log_error("Creating Update Package", mhc_ErrorMessage);
         s32_Return = C_RANGE;
      }
   }
   ...
   if (s32_Return == C_NO_ERR)
   {
      C_SclString c_TmpPackagePath = orc_PackagePath.SubString(1, orc_PackagePath.Pos(mhc_PACKAGE_EXT) - 1) +
                                     mhc_PACKAGE_EXT_TMP;
      // add trailing path delimiter to temporary folder if not present
...
*/
//----------------------------------------------------------------------------------------------------------------------
int32_t C_OscSupServiceUpdatePackageCreate::mh_CreateDeviceIniFile(const C_SclString & orc_Path,
