...
static const stw_types::uint16 CXFLFLASHWRITE_VERSION = 0x1020U;
...
enum E_XFLEraseMode ///< determines which flash sectors shall be erased before flashing
{
   ...
   eXFL_ERASE_MODE_AUTOMATIC            ///< automatically detect which sectors to erase (only from protocol version V3.00r0 on)
};
...
enum E_XFLDevTypeCheckGetIDFailReaction
{
   ...
   eXFL_DEV_TYPE_CHECK_GETID_FAIL_REACTION_ASK    = 2    ///< ask user whether to continue  -> no x-check if he/she selects "yes"
};
...
public:
   ...
   stw_types::uint16 u16_Version;   ///< used to check consistency between header-file and library, use CXFLWRITEFLASH_VERSION
   ...
   stw_types::uint16 u16_InterFrameDelayUs; ///< delay in us between individual frames when sending hex-lines.
                                           ///< helpful for low-performance targets at high bitrates.
                                           ///< set to 0 for most communication interfaces and target nodes

   stw_types::uint8  u8_HexRecordLength;  ///< patch hexfile records to different length; 0 = don't modify

   bool q_XtdID;                     ///< false: use 11bit CAN message IDs; true: use 29bit CAN message IDs
   stw_types::uint32 u32_SendID;      ///< CAN message TX ID used in flashloader protocol
   stw_types::uint32 u32_ReceiveID;   ///< CAN message RX ID used in flashloader protocol (usually TX-ID + 1).
   ...
   stw_types::uint8 u8_DivertTargetIndex; ///< slot of babyboard B
   stw_types::uint8 au8_DivertUserID[2];  ///< key to be able to write to ESX2 babyboard B
   ...
   stw_types::uint8 u8_IgnoreInvalidHexfileError; ///< behaviour on erroneous hexfile  \n
                                       ///< 0 -> always fail  \n
                                       ///< 1 -> always ignore \n
