...
sint32 C_OSCSecurityPemDatabase::ParseFolder(const std::string & orc_FolderPath)
{
   ...
   if (TGL_DirectoryExists(orc_FolderPath))
   {
      const std::vector<std::string> c_Files = C_OSCSecurityPemDatabase::mh_GetPEMFiles(orc_FolderPath);
      ...
      osc_write_log_info("Read PEM database",
                         ...
                         stw_scl::C_SCLString::IntToStr(
                            c_Files.size()) + " PEM files in folder \"" + orc_FolderPath + "\".");
      //Reset error status
      s32_Retval = C_NO_ERR;
   }
   else
