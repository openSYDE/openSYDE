...
public:
   ...
   static stw_scl::C_SCLString GetMD5(const stw_types::uint8 * const pBuf, const stw_types::uint32 nLength);
   static stw_scl::C_SCLString GetMD5(FILE * const opt_File);
   static stw_scl::C_SCLString GetMD5(const stw_scl::C_SCLString & strFilePath);
...
protected:
   ...
   void Transform(const stw_types::uint8 Block[64]);
   void Update(const stw_types::uint8 * Input, const stw_types::uint32 nInputLen);
   stw_scl::C_SCLString Final();
   stw_types::uint32 RotateLeft(const stw_types::uint32 x, const stw_types::sint32 n);
   void FF(stw_types::uint32& A, const stw_types::uint32 B, const stw_types::uint32 C, const stw_types::uint32 D,
		   const stw_types::uint32 X, const stw_types::uint32 S, const stw_types::uint32 T);
   void GG(stw_types::uint32& A, const stw_types::uint32 B, const stw_types::uint32 C, const stw_types::uint32 D,
		   const stw_types::uint32 X, const stw_types::uint32 S, const stw_types::uint32 T);
   void HH(stw_types::uint32& A, const stw_types::uint32 B, const stw_types::uint32 C, const stw_types::uint32 D,
		   const stw_types::uint32 X, const stw_types::uint32 S, const stw_types::uint32 T);
   void II(stw_types::uint32& A, const stw_types::uint32 B, const stw_types::uint32 C, const stw_types::uint32 D,
           const stw_types::uint32 X, const stw_types::uint32 S, const stw_types::uint32 T);

   //utility functions
   void DWordToByte(stw_types::uint8 * const Output, const stw_types::uint32* const Input,
                    const stw_types::uint32 nLength);
   void ByteToDWord(stw_types::uint32* const Output, const stw_types::uint8 * const Input,
                    const stw_types::uint32 nLength);

private:
   stw_types::uint8  m_lpszBuffer[64]; //input buffer
   stw_types::uint32 m_nCount[2];      //number of bits, modulo 2^64 (lsb first)
   stw_types::uint32 m_lMD5[4];        //MD5 checksum
};
