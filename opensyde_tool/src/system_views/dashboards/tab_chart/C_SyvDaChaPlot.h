...
public:
   ...
   virtual ~C_SyvDaChaPlotTriangleItem(void);

   void UpdateColor(const QColor & orc_Color);

   //lint -e{8001,8011,9272}  //name of function and parameters dictated by base class
   virtual stw_types::float64 selectTest(const QPointF & orc_Pos, bool oq_OnlySelectable,
                                         QVariant * const opc_Details) const override;

   QCPItemPosition * const pc_Position;

protected:
   //lint -e{8001,8011,9272}  //name of function and parameters dictated by base class
   virtual void draw(QCPPainter * const opc_Painter) override;
...
public:
   ...
   virtual ~C_SyvDaChaPlotCursorTag(void);

   void UpdateColors(const QColor & orc_LabelColor, const QColor & orc_LineColor);
   void UpdatePosition(const stw_types::float64 of64_PosX);
...
public:
   C_SyvDaChaPlotCursorItem(QCustomPlot * const opc_ParentPlot, const stw_types::float64 of64_InitXPos,
                            const QString & orc_LabelText);
   virtual ~C_SyvDaChaPlotCursorItem(void);
...
Q_SIGNALS:
   ...
   void SigCursorItemMovedOnXAxis(const C_SyvDaChaPlotCursorItem * const opc_Line, const stw_types::float64 of64_PosX);
