...
*/
//----------------------------------------------------------------------------------------------------------------------
C_SyvDaItPaWidgetNew::C_SyvDaItPaWidgetNew(const uint32_t & oru32_ViewIndex,
...
*/
//----------------------------------------------------------------------------------------------------------------------
bool C_SyvDaItPaWidgetNew::HandleManualOperationFinished(const int32_t os32_Result, const uint8_t ou8_Nrc)
{
   const bool q_Return = this->mq_ReadActive;
   int32_t s32_Result = os32_Result;
            ...
            if ((s32_Result == C_NO_ERR) || (s32_Result == C_CHECKSUM))
            {
               ...
               else
               {
                  if (this->mc_ListsWithCrcError.size() > 0UL)
                  {
                     ...
                     for (uint32_t u32_ItList = 0; u32_ItList < this->mc_ListsWithCrcError.size(); ++u32_ItList)
                     {
...
void C_SyvDaItPaWidgetNew::ButtonAddClicked(void)
{
   ...
   if (pc_ParamWidget != NULL)
   {
      ...
      if (c_New->exec() == static_cast<int32_t>(QDialog::Accepted))
      {
         ...
         for (uint32_t u32_ItNewElement = 0; u32_ItNewElement < c_DataElements.size(); ++u32_ItNewElement)
         {
            ...
            if (pc_Param != NULL)
            {
               ...
               for (uint32_t u32_ItElement = 0; u32_ItElement < pc_Param->c_DataPoolElementsConfig.size();
                    ++u32_ItElement)
               {
...
*/
//----------------------------------------------------------------------------------------------------------------------
uint32_t C_SyvDaItPaWidgetNew::GetSelectedItemCount(void) const
{
...
void C_SyvDaItPaWidgetNew::m_ReadElements(void)
{
   ...
   if (this->mpc_ComDriver != NULL)
   {
      ...
      {
         for (uint32_t u32_ItList = 0UL; u32_ItList < this->mc_ListIds.size(); ++u32_ItList)
         {
...
void C_SyvDaItPaWidgetNew::m_WriteElements(const std::vector<C_OscNodeDataPoolListElementId> & orc_ChangedIds)
{
   ...
   for (uint32_t u32_ItChanged = 0UL; u32_ItChanged < c_ChangedElementsStart.size(); ++u32_ItChanged)
   {
      ...
      bool q_Found = false;
      for (uint32_t u32_ItSelected = 0UL; (u32_ItSelected < orc_ChangedIds.size()) && (q_Found == false);
           ++u32_ItSelected)
      {
   ...
   for (uint32_t u32_ItInvalid = 0UL; u32_ItInvalid < c_InvalidLists.size(); ++u32_ItInvalid)
   {
         ...
         if (this->mpc_Ui->pc_TreeView->CheckRange(c_InterestingChangedElements, c_InterestingInvalidLists) == true)
         {
            ...
            if (this->mpc_ComDriver != NULL)
            {
               ...
               else
               {
                  ...
                  for (uint32_t u32_ItId = 0; u32_ItId < c_InterestingChangedElements.size(); ++u32_ItId)
                  {
                  ...
                  for (uint32_t u32_ItId = 0; u32_ItId < c_InterestingInvalidLists.size(); ++u32_ItId)
                  {
               ...
               if (c_New->exec() == static_cast<int32_t>(QDialog::Accepted))
               {
                  ...
                  if (c_OutputListIds.size() == c_OutputContent.size())
                  {
                     ...
                     if (pc_ParamData != NULL)
                     {
                        ...
                        for (uint32_t u32_ItOutput = 0; u32_ItOutput < c_OutputListIds.size(); ++u32_ItOutput)
                        {
                           //Search in param widget
                           for (uint32_t u32_ItConfig = 0UL;
                                u32_ItConfig < pc_ParamData->c_DataPoolElementsConfig.size();
                                ++u32_ItConfig)
                           {
                              const C_PuiSvDbNodeDataElementConfig & rc_Config =
      ...
      for (u32_ListCounter = 0U; u32_ListCounter < orc_ListIds.size(); ++u32_ListCounter)
      {
             ...
             (rc_CurElementId.u32_ListIndex < pc_SdNode->c_DataPools[rc_CurElementId.u32_DataPoolIndex].c_Lists.size()))
         {
            ...
            if (q_NodeFound == false)
            {
               ...
               u32_CurIntNodeIndex = static_cast<uint32_t>(c_IntNodes.size() - 1);
               pc_CurIntNode = &c_IntNodes[u32_CurIntNodeIndex];
            ...
            if (pc_CurIntNode != NULL)
            {
               ...
               if (q_DataPoolFound == false)
               {
                  ...
                  u32_CurIntDataPoolIndex = static_cast<uint32_t>((pc_CurIntNode->c_DataPools.size() - 1));
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SyvDaItPaWidgetNew::mh_GetDefaultFileName(const uint32_t ou32_ViewIndex,
                                                    const C_OscNodeDataPoolListElementId & orc_Id,
...
*/
//----------------------------------------------------------------------------------------------------------------------
QString C_SyvDaItPaWidgetNew::mh_GetFile(const C_OscNodeDataPoolListElementId & orc_Id, const uint32_t ou32_Value)
{
   QString c_Retval;
