                          ...
                          const QModelIndex & orc_Index) const
{
   ...
   {
      const QFont c_FontHeading("Segoe UI", 12, static_cast<int32_t>(QFont::Normal));
      const QFontMetrics c_FontHeadingInfo(c_FontHeading);
      const int32_t s32_Height = orc_Option.rect.height();
      ...
      //Display heading
      {
         const QString c_Heading = orc_Index.data(static_cast<int32_t>(Qt::DisplayRole)).toString();
         c_Rect = orc_Option.rect.adjusted(0, 0, 0, c_FontHeadingInfo.height() - s32_Height);
         opc_Painter->setFont(c_FontHeading);
         opc_Painter->drawText(c_Rect, static_cast<int32_t>(Qt::AlignLeft), c_Heading);
      }
      //Display description
      {
         const QString c_Description = orc_Index.data(ms32_USER_ROLE_ADDITIONAL_INFORMATION).toString();
         c_Rect = orc_Option.rect.adjusted(0, c_FontHeadingInfo.height(), 0, 0);
         opc_Painter->setFont(QFont("Segoe UI", 10, static_cast<int32_t>(QFont::Normal)));
         opc_Painter->drawText(c_Rect, static_cast<int32_t>(Qt::AlignLeft), c_Description);
      }
      //advisory
      opc_Painter->restore();
   }
