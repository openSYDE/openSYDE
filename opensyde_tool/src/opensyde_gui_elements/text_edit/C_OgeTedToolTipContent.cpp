...
void C_OgeTedToolTipContent::m_AdjustMinimumSize()
{
   ...
   QFontMetrics c_FontMetric(c_Font);
   ...
   if (s32_LongestTextLength < 100)
   {
      s32_Width = static_cast<sint32>(c_FontMetric.boundingRect(c_LongestText).width() +
                                      (2 * this->frameWidth())) + 6; // extra 6 for bigger letters in a second line with
   ...
   s32_Height = static_cast<sint32>(c_FontMetric.boundingRect
                                       (0, 0, s32_Width, 0, static_cast<uint32>(Qt::TextWrapAnywhere),
                                       c_Text).height() +
                                    (2 * this->frameWidth())) + 2;
