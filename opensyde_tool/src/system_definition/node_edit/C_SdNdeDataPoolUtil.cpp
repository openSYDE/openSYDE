...
*/
//----------------------------------------------------------------------------------------------------------------------
void C_SdNdeDataPoolUtil::h_ConvertToElementGeneric(
   const stw_opensyde_core::C_OSCNodeDataPoolListElement & orc_OSCElement,
   const C_PuiSdNodeDataPoolListElement & orc_UIElement, const C_SdNdeDataPoolUtil::E_ElementDataChangeType & ore_Type,
   QVariant & orc_Generic, const uint32 & oru32_Index, const sint32 & ors32_DataSetIndex)
{
   ...
   case eELEMENT_AUTO_MIN_MAX:
      ...
      break;
   case eELEMENT_MIN:
      orc_Generic = C_SdNdeDataPoolContentUtil::h_ConvertContentToGeneric(orc_OSCElement.c_MinValue, oru32_Index);
      break;
   case eELEMENT_MAX:
      orc_Generic = C_SdNdeDataPoolContentUtil::h_ConvertContentToGeneric(orc_OSCElement.c_MaxValue, oru32_Index);
      break;
   ...
   case eELEMENT_DATA_SET:
          ...
          (static_cast<uint32>(ors32_DataSetIndex) < orc_OSCElement.c_DataSetValues.size()))
      {
         ...
         {
            orc_Generic =
               C_SdNdeDataPoolContentUtil::h_ConvertContentToGeneric(orc_OSCElement.c_DataSetValues[static_cast<uint32>(
                                                                                                       ors32_DataSetIndex)],
                                                                     oru32_Index);
         }
      }
      ...
      else if (static_cast<uint32>(pc_Datapool->c_Lists.size() - 1) == ou32_ListIndex)
      {
         ...
         if (osn_MaximumHeight > 0)
         {
            ...
            for (u32_ListCounter = 0U; u32_ListCounter < (pc_Datapool->c_Lists.size() - 1UL); ++u32_ListCounter)
            {
               ...
               sn_HeightOtherLists +=
                  static_cast<sintn>(C_SdNdeDataPoolUtil::h_GetTableSize(ou32_NodeIndex, ou32_DataPoolIndex,
                                                                         u32_ListCounter)) +
                  sn_HeaderOffset;
            }
...
*/
//----------------------------------------------------------------------------------------------------------------------
sint32 C_SdNdeDataPoolUtil::GetSharedDatapoolGroup(const uint32 ou32_SharedDatapoolGroup,
