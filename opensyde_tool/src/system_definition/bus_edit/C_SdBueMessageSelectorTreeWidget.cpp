   ...
   if (c_IndexList.size() > 0)
   {
      ...
      if ((q_MessagesOnly == true) && (this->mpc_MessageSyncManager != NULL))
      {
         ...
         C_SdClipBoardHelper::h_StoreMessages(c_Messages, c_OSCSignalCommons, c_UISignalCommons, c_UISignals,
                                              c_OwnerNodeName, c_OwnerNodeInterfaceIndex, c_OwnerIsTxFlag);
      }
         ...
         if (c_NodeIndexes.size() == c_InterfaceIndexes.size())
         {
            for (uint32 u32_ItNode = 0; u32_ItNode < c_NodeIndexes.size(); ++u32_ItNode)
            {
               const C_OSCCanMessageContainer * const pc_MessageContainer =
                  C_PuiSdHandler::h_GetInstance()->GetCanProtocolMessageContainer(c_NodeIndexes[u32_ItNode],
                                                                             ...
                                                                             uint32 & oru32_InterfaceIndex) const
{
   ...
   else
   {
      ...
      C_PuiSdNodeCanMessageSyncManager::h_GetConnectedAndActiveInterfaces(this->mu32_BusIndex,
                                                                          ...
                                                                          c_NodeIndexes,
                                                                          c_InterfaceIndexes);
      if ((c_NodeIndexes.size() == c_InterfaceIndexes.size()) && (c_NodeIndexes.size() > 0))
      {
...
sint32 C_SdBueMessageSelectorTreeWidget::GetMessageIdForAdd(C_OSCCanMessageIdentificationIndices & orc_MessageId) const
{
   ...
   if (this->m_GetFirstConnectedNodeAndInterface(u32_NodeIndex, u32_InterfaceIndex) == C_NO_ERR)
   {
      const C_OSCCanMessageContainer * const pc_MessageContainer =
...
{
   // CAN message for "testing" the signals in its positions
   C_OSCCanMessage c_Message = *C_PuiSdHandler::h_GetInstance()->GetCanMessage(orc_MessageId);
   uint32 u32_Counter;

   for (u32_Counter = 0U; u32_Counter < orc_Signals.size(); ++u32_Counter)
   {
      orc_Signals[u32_Counter].u16_ComBitStart = C_SdBueMessageSelectorTreeWidget::mh_GetStartBit(
         &c_Message, orc_Signals[u32_Counter].u16_ComBitLength, orc_Signals[u32_Counter].e_ComByteOrder);

      // add the signal for getting the position of the next signal
      c_Message.c_Signals.push_back(orc_Signals[u32_Counter]);
   }
}
